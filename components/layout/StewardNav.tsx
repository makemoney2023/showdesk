...
import { stewardNavItemActive, stewardNavItems } from "@/lib/domain/role-shell";
...
export function StewardNav({
  ...
  return (
    <nav
      className="sss-paper fixed inset-x-0 bottom-0 z-50 flex min-h-[calc(3.75rem+env(safe-area-inset-bottom))] justify-around rounded-none border-x-0 border-b-0 pt-1.5 pb-[env(safe-area-inset-bottom)]"
      aria-label="Ringside"
    >
      {items.map((item) => {
        const active = stewardNavItemActive(item.href, activeHref);
